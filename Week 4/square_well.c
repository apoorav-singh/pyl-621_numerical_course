// Sum of the series (1/n^4) is computed in this program.
...
void main()
{
	...
	act_s = pow(pi,4)/96; //(480*hbar*hbar)/(pow(pi,4)*m*a)*pow(pi,4)/96;
...
// For printing the forward summastion
	...
	{	
		...
		{
			n1 = i;
			sum1[i] =sum1[i-1] +  (1/pow(n1,4)); //(480*hbar*hbar)/(pow(pi,4)*m*a)*(1/pow(n1,4)); 
			//printf("%lf\n",sum1[i]);
		}
...
// For printing the backward summastion
	...
	{	
		...
		{
			n1 = i;
			sum2[i] = sum2[i+1] + (1/pow(n1,4));// (480*hbar*hbar)/(pow(pi,4)*m*a)*(1/pow(n1,4));
			//printf("%lf\n",sum2[i]);
		}
